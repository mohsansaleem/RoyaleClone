...
// For reference: http://unitypatterns.com/resource/objectpool/
...
//		Debug.Log ("Recycling: "+obj.name);
    ...
    {
        DestroyPooled(prefab.gameObject,0);
    }

	public static void DestroyPooled(GameObject prefab,int Count)
	{
        List<GameObject> pooled;
        if (instance.pooledObjects.TryGetValue(prefab, out pooled))
        {
            Count = Count == 0 ? pooled.Count : Count; 
			for (int i = 0; i < Mathf.Min(pooled.Count,Count); ++i)
            {
				GameObject.Destroy(pooled[i]);
                pooled.Remove(pooled[i]);
            }
		}
	}
	public static void DestroyPooled<T>(T prefab,int count) where T : Component
	{
        DestroyPooled(prefab.gameObject,count);
	}
